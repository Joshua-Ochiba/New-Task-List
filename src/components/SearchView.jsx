...
export default function SearchView() {
  ...
  const selectedTask = searchResults?.find(
    (task) => task._id === selectedTodoId
  );
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          {searchTerm.trim() ? (
            ...
            <div className="p-8 text-center text-gray-500">
              ...
              <p className="text-sm">
                Enter a search term to find your tasks by title, content, or
                tags
              </p>
            </div>
          )}
      ...
      {!isMobile && (
        <div className="flex-1 min-w-0 bg-white">
          {selectedTask ? (
            <TaskPreview
              task={selectedTask}
              onEdit={() => navigate(`/todo/${selectedTask._id}`)}
            />
          ) : (
